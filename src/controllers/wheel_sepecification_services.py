...
def submit_wheel_specification_api(payload: AddWheelSpecificationPayload, db: Session):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail="Failed to submit wheel specification.")
...
def get_wheel_specifications_api(payload: GetWheelSpecificationQuery, db: Session):
    ...
    try:
        ...
        if payload.submittedDate:
            query = query.filter(WheelSpecification.submittedDate == payload.submittedDate)

        results = query.all()

        if not results:
            raise HTTPException(status_code=404, detail="No wheel specifications found for the given filters.")

        data = []
        for item in results:
            try:
                fields_data = json.loads(item.fields) if isinstance(item.fields, str) else item.fields
        ...
        raise HTTPException(status_code=500, detail="Failed to fetch wheel specifications.")
